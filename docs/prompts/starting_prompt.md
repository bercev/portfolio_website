Let’s go for something crazy and big. Invoke the frontend UI design skills, claude superpowers, and any brainstorming skills you have to break this down. Spawn as many subagents as you need and create clean code (reoragnize files when necessary) which each component living in a dedicated file. I’ve uploaded and added links to everything as well. Use the 21st.dev MCP and context7 mcp for research. 

1. Use to grab attention and display the works on my resume and career history. 
2. Ill upload my resume (resume.pdf) and provide links: My github: https://github.com/bercev,  linkedin: https://linkedin.com/in/berat-ercevik, Look at my Linkedin, if possible, for other information. also here’s a short bio: Berat is a software engineer with knowledge of a variety of programming languages, frameworks, and CS fundamentals. His practical experience consists of building full-stack applications, developing agentic systems, and conducting AI research. He is completing his undergraduate degree in Computer Science at UC Santa Cruz. Also, visit https://vitae.tools/ and take some screenshots possibly of the website there to demonstrate it if you want. (its a resume history item)
3. One page, i want scroll animations definetley and parallax effects. I’m planning on using 21st.dev, shadcnui, framer motion etc. Also, verify the site using playwright and actually test it out.
4. I want both light and dark mdoe. The theme I want is from 21st.dev and its this: Chalk Slate by Serafim (i think themes should be fairly hot swappable)
5. For animations, use motion.dev. Set it up by doing these commands: `npm install motion` and `npx motion-ai` to set it up for yourself
Try to implement something like this perhaps? https://motion.dev/examples/react-magnetic-filings
https://motion.dev/examples/react-confetti (when you dwonload my resume)
https://motion.dev/examples/react-radial-menu (for idk something? maybe socials)
https://motion.dev/examples/react-dots-morph-button (no idea)
https://motion.dev/examples/react-scroll-text-lines (for text)
https://motion.dev/examples/react-scramble-text-stagger-center (for more text, maybe the big title hero)
https://motion.dev/examples/react-multi-state-badge (for download button aswel))
https://21st.dev/@manuarora700/components/glowing-effect (for any cards) 

For compontents, i want a cool mouse trail effect. whatever you can find thats modern and minimialistic and works well. perhaps lik ea liquid glass effect.  and this component for the main background.  PROMPT: 
...
A mouse trail effect: import React, { useEffect, useRef } from 'react';
...
Another mouse traile ffect, use whichver works better: "use client";
