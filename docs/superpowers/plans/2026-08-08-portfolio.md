# Implementation Plan: Berat Ercevik One-Page Portfolio

**Date:** 2026-08-08
**Spec:** `docs/superpowers/specs/2026-08-08-portfolio-design.md`
**Source brief:** `docs/prompts/starting_prompt.md`
**Stack:** Next.js 16.3.0 (App Router) · React 19 · TypeScript (strict) · Tailwind CSS v4 · `motion` v13 · shadcn/ui · 21st.dev (`GlowingEffect`) · `threejs-components` · `next-themes` · Playwright.

## For agentic workers

Work top-to-bottom through the tasks. Each task produces working code and you may commit after each task. Do NOT skip the verification steps — they are how you know the task actually works. If a command prompts interactively, use the non-interactive flags shown. If an external registry component is paywalled/missing, hand-roll the equivalent (never block).

## Architecture (from spec §7)
...
e2e/
  portfolio.spec.ts       # Playwright verification (spec §9)
...
**Design tokens:** `data/content.ts` → section components. No runtime data fetching.

---

## Task 0 — Scaffold: dependencies, resume copy, config
...
4. Verify `threejs-components` exports a TubesCursor factory:
   ...
   Expect `tubes1.min.js` (ES module, default export = `(canvas, opts) => app`). If the main package export works too, prefer `import(...)` of the subpath used below.
...
## Task 1 — shadcn/ui init + primitives
...
1. Init shadcn (non-interactive; Tailwind v4 + new-york style):
   ...
   - If it prompts, answer: new-york, neutral, CSS variables yes. It generates `components.json`, `lib/utils.ts`, and a starter `globals.css` (we override in Task 2).
2. Add primitives:
...
## Task 2 — Chalk Slate theme, dark mode, providers, metadata
...
2. Write `app/globals.css` as (Tailwind v4 CSS-first):
   ```css
   @import "tailwindcss";
...
3. Write `components/providers.tsx`:
   ```tsx
   "use client";
...
**Verify:** `npm run dev`; page has no FOUC, `<html class="dark">` toggles with OS scheme; `next build` compiles (may fail only until later tasks if imports are missing — then run after Task 8).
...
Write the full typed content (this is the single source of truth; copy verbatim from the resume):
...
**Note on email:** The source material has no public email; use a `mailto:` only if a real address is added. Default: Contact section links GitHub + LinkedIn + a resume download. Do NOT invent an email.

**Verify:** `npx tsc --noEmit` passes (or `npm run build` in a later task).

---

## Task 4 — Effects (motion) components
...
### 4.1 `scramble-text.tsx`
Scramble + stagger for the hero headline (equivalent of `react-scramble-text-stagger-center`).
...
### 4.2 `magnetic-button.tsx`
Magnetic hover wrapper (equivalent of `react-magnetic-filings`).
...
### 4.3 `scroll-text-lines.tsx`
Section-heading reveal: masked line-slide-up + a gentle scroll-linked shift (equivalent of `react-scroll-text-lines`).
...
### 4.4 `radial-menu.tsx`
Hero socials radial menu (desktop only; the footer/contact inline row is the mobile/a11y fallback).
...
### 4.5 `confetti-button.tsx`
Resume download with multi-state (Idle → Downloading → Done) + confetti.
...
### 4.7 `back-to-top.tsx`
Dots-morph → chevron, floating bottom-right, appears after scroll.
...
**Verify:** `npx tsc --noEmit` passes after Task 4.

---

## Task 5 — Liquid background (TubesCursor)
...
## Task 6 — Glowing effect (21st.dev) + glow card
...
Each section is a server component unless it needs client state. Section files: `app/` is thin; put sections in `components/sections/*.tsx`.
...
```
(Add `"use client"` at top — hero uses motion; or wrap the bounce in a small client child. Simplest: `"use client"` on hero.)
...
```
Add the marquee keyframes in `globals.css`:
```css
@keyframes marquee { from { transform: translateX(0); } to { transform: translateX(-50%); } }
.animate-marquee { animation: marquee 30s linear infinite; }
```
(Gated to run always; if reduced-motion matters, it is subtle — acceptable. Optionally `@media (prefers-reduced-motion: reduce) { .animate-marquee { animation: none; } }`.)
...
```
(Use a static year literal `2026` to avoid hydration mismatch — do NOT call `new Date()` in a server component that must match client; since this is a server component, it's fine, but keep it simple with `2026`.)
...
**Verify:** each section renders at its anchor; header/nav/theme-toggle work; `tsc --noEmit` clean.
...
## Task 9 — Manual + Playwright verification (spec §9)
...
**Verify:** all tests pass; screenshots reviewed; light + dark look right; confetti fires on download.

---

## Task 10 — Final commit + handoff

1. `git add -A` and commit with a message summarizing the build.
2. Report: what was built, verification results, and any deviations from spec.

---

## Global constraints (all tasks)

- **motion imports** always come from `"motion/react"` (never `framer-motion`).
...
- **No secrets:** nothing beyond public resume content; the anonymous employer stays "Stealth Startup".
